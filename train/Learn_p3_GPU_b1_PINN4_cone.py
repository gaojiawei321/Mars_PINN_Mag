...
inp_idx = [0,1,2,6,3,7,8,9]     # x,y,z,  Psw, sw_b, Bx By Bz
...
# 保存
STATS_PATH = "../save/train_norm_stats_L1_b1_p43.npz"
np.savez(STATS_PATH,
...
def imf_vec_from_swb_cone(swb, cone_deg, device):
#    th = torch.deg2rad(torch.as_tensor(cone_deg, dtype=torch.float32, device=device))

    th = torch.as_tensor(cone_deg, dtype=torch.float32, device=device)

    bdir = torch.stack([torch.cos(th), torch.sin(th), torch.zeros_like(th)], dim=-1)
    return swb.unsqueeze(-1)*bdir   # [N,3]
...
#        imfb_phys = batch[:, 4]  * std[4] + mean[4]
        ...
        # 反归一化取 sw_b 与 Cone（注意列索引：sw_b 在第 5 列(0-based=4)，Cone 在第 7 列(=6)）
        sw_b_phys = batch[:, 4] * std[4] + mean[4]
        cone_deg = batch[:, 3] * std[3] + mean[3]
        #        B_imf     = imf_from_cone_and_mag(sw_b_phys, cone_deg, device=device)  # [N,3]
        B_imf = imf_vec_from_swb_cone(sw_b_phys, cone_deg, device=device)  # [N,3]
...
#              f"RelL2 Train: {rel_l2_train:.4f}, RelL2 Val: {rel_l2_val:.4f}")
    ...
    if avg_val_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), '../save/best_model_b1_p43.pth')
        patience_counter = 0
    else:
        patience_counter += 1
...
pd.DataFrame(history).to_csv("../save/training_history_V3_L1_500_b1_p43.csv", index=False)
print("Loss history saved to training_history.csv")

# 保存

torch.save(model.state_dict(), '../save/pinn_model_VSE_v3_L1_500_b1_p43.pth')
